...
public class RoundRobinStormScheduler implements IScheduler {
    ...
    Map conf;
    Map<String, List<SupervisorDetails>> componentToSupervisor = new HashMap<String, List<SupervisorDetails>>();
    ...
    private void indexSupervisors(Collection<SupervisorDetails> supervisorDetails) {
        for (SupervisorDetails supervisor : supervisorDetails) {
            ...
            if (componentId != null && !componentId.isEmpty()) {
                List<SupervisorDetails> supervisorList = componentToSupervisor.get(componentId);
                if (supervisorList == null) {
                    supervisorList = new ArrayList<SupervisorDetails>();
                    componentToSupervisor.put(componentId, supervisorList);
                }
        ...
        if (siddhiTopology != null) {
            ...
            if (!isSchedulingNeeded) {
                ...
                for (Map.Entry<String, List<ExecutorDetails>> entry : componentToExecutors.entrySet()) {
                    ...
                    List<SupervisorDetails> supervisorList = componentToSupervisor.get(componentName);
                    if (supervisorList != null && !supervisorList.isEmpty()) {
                        ...
                        int supervisorCount = supervisorList.size();
